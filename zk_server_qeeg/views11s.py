...
# 2025/10/21 V2.4 ABP/RBP读入数据及时间分片优化
# view11s 暂停更新
...
def multi_chns_csa(start_seconds, stop_seconds, raw, ttype, channels, fs, task_id, band=(0.5, 30), window=2.0, overlap=0.8, nfft=None, log_power=True):
    size = int(window * fs)
    ...
    train_data = parallel_chns_split(start_seconds, stop_seconds, step, raw, channels, n_jobs=n_job)
...
def plot_csa(rbp_features, tmp_data_path, ttype, fig_name):
    plt.figure(figsize=(12, 4))
...
if __name__ == '__main__':
    ...
    if is_history == 1:
        ...
        for fname in os.listdir(file_path):  # 寻找.rml文件
            ...
            # channel_values =multi_chns_csa(start_seconds, stop_seconds, raw, 'CSA', ['Fp1'], sample_frequency, '123456')
            for chn in range(len(channel_values)):
                # RBP波形
                plot_csa(channel_values[chn], '/disk1/workspace/py39_tf270/SleepEpilepsy1/resource/test/sleepstage/JJY/', 'CSA', raw.ch_names[chn] + '_csa.png')
            ...
            print("SEF计算完成，共运行：%.8s s" % (time.time() - start_time1))
        ...
        ttype = trend_name[2]  # 1 2 3 4 5 6 9 10
