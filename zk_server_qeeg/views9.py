...
def qteeg_monitor(request):  # 待验证
    if request.method == 'POST':
        if request.content_type == 'application/json':
            ...
            start_seconds, stop_seconds, raw, channels, sample_frequency, 'abp', input_data_dict["taskID"]))
...
def multi_chns_rav(start_seconds, stop_seconds, raw, channels, fs, task_id, alpha_band=(6,14), alpha_band1=(1,20), window=2.0, overlap=0, nperseg=None):
...
def multi_chns_env(start_seconds, stop_seconds, raw, channels, fs, history_monitor, task_id):
    train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
    ...
    if train_data.shape[1] > 1:
        for segment in train_data:
            source_eeg = sosfilt(sos, segment*unit_coef)
        ...
        for segment in train_data:
            ...
            aeeg_output = 1.631 * np.abs(eeg_filtered) + 4
            channel_values.append(np.percentile(aeeg_output, 50))
...
def multi_chns_tp(start_seconds, stop_seconds, raw, channels, fs, history_monitor, task_id, band=(0.5, 50), nperseg=None):
    train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
...
def multi_chns_adr_abr(start_seconds, stop_seconds, raw, channels, fs, ar_type, task_id, alpha_band=(8, 13), delta_band=(0.5, 4), beta_band=(13,30), nperseg=None):
    train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
...
def multi_chns_sef(start_seconds, stop_seconds, raw, channels, fs, task_id, band=(0.5, 40), edge=0.9, nperseg=None):
    train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
