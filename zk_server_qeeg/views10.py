...
# 2025/10/11 V1.8 TP、ADR、ABR、RAV、Envolpe 回读多线程测试通过
...
def multi_chns_rav(start_seconds, stop_seconds, raw, ttype, channels, fs, task_id, alpha_band=(6,14), alpha_band1=(1,20), window=2.0, overlap=0, nperseg=None):
    if stop_seconds - start_seconds < 120:  # 2 min
        ...
        if train_data.shape[1] > 1:
            ...
            for segment in train_data:
                alpha_powers, alpha_powers1 = [], []
                for start in range(0, len(segment) - size + 1, step):
                    seg = segment[start:start + size]
                    freqs, psd = welch(seg, fs=fs, nperseg=size)
                    idx_alpha = np.logical_and(freqs >= alpha_band[0], freqs <= alpha_band[1])
                    idx_alpha1 = np.logical_and(freqs >= alpha_band1[0], freqs <= alpha_band1[1])
                ...
                if len(alpha_powers) < 2:
                    ...
                    channel_values.append(mean_alpha * 100 / mean_alpha1)
...
def multi_chns_env(start_seconds, stop_seconds, special_electrodes, raw, ttype, channels, fs, history_monitor, task_id):
    ...
    if train_data.shape[1] > 1:
        for segment in train_data:
            source_eeg = sosfilt(sos, segment)
        ...
        channel_values1 = np.array(channel_values1)
        for i in range(train_data.shape[1]):
            env_tmp = np.round(channel_values1[:, i], 2)
        ...
        for segment in train_data:
            ...
            channel_values.append(round(np.percentile(aeeg_output, 50), 2))

        channel_values = np.array(channel_values)
        channel_values = channel_values.reshape((len(channel_values), 1))
        channel_values1 = []
        for tk in channel_values:
            channel_values1.append(tk.tolist())
        channel_values1 = [channel_values1]
        # print("channel_values1: ", channel_values1)

    ana_type = ttype
    tk = str(channels)
    tk = tk.replace("\'", "\"")
    result = "{ " + f"\"taskID\": \"{task_id}\", \"type\": \"{ana_type}\", \"labels\": {tk}, \"values\": {channel_values1}" + " }"

    return result

def multi_chns_tp(start_seconds, stop_seconds, raw, ttype, channels, fs, history_monitor, task_id, band=(0.5, 50), nperseg=None):
    train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
...
def multi_chns_adr_abr(start_seconds, stop_seconds, raw, ttype, channels, fs, ar_type, task_id, alpha_band=(8, 13), delta_band=(0.5, 4), beta_band=(13,30), nperseg=None):
    ...
    if train_data.shape[1] > 1:
        for segment in train_data:
            ...
            # 3. 防止除零
            adr = np.where(bd_power > 0, alpha_power / bd_power * 100, 0)
        ...
        for segment in train_data:
            ...
            # 3. 防止除零
            adr = alpha_power / bd_power * 100 if bd_power > 0 else 0
            channel_values.append(adr)
...
def multi_chns_sef(start_seconds, stop_seconds, raw, ttype, channels, fs, task_id, band=(0.5, 40), edge=0.9, nperseg=None):
    train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
...
if __name__ == '__main__':
    is_history = 4  # 1-history 0-monitor 2-abp 3-rbp 4-env
    ...
    if is_history == 1:
        ...
        stop_seconds = 600
        special_electrodes = []
        raw = mne.io.read_raw_bdf(os.path.join("/disk1/workspace/py39_tf270/SleepEpilepsy1/resource/test/sleepstage/20250912110203", "20250912110203.bdf"), include=tuple(['T3', 'Fp1', 'F3']))
        ttype = trend_name[2]  # 2 6 10
        channel_values = abp_rbp_com_t(
            start_seconds, stop_seconds, raw, ttype, ['Fp1', 'F3'], int(raw.info['sfreq']), 'rbp',
            '123456')
        # channel_values = multi_chns_env(start_seconds, stop_seconds, special_electrodes, raw, ttype, ["Fp1-REF", 'F3-REF'],
        #     int(raw.info['sfreq']), 'history', '123456')
        # channel_values = multi_chns_sef(start_seconds, stop_seconds, raw, ttype, ['T3'], int(raw.info['sfreq']),
        #     '123456')
