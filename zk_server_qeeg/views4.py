...
from scipy.signal import butter, remez, lfilter, filtfilt, kaiser_atten, kaiser_beta, firwin, welch
import matplotlib.pyplot as plt
# ssh root@172.24.60.106
# conda activate py39_tf270_torch1101
# cd /disk1/workspace/py39_tf270/zk_server/
# export LD_LIBRARY_PATH=/usr/local/cuda-11.2/lib64:$LD_LIBRARY_PATH
# python manage.py runserver 0.0.0.0:8041 --noreload  localhost:4567 192.168.1.100
# import sys
import os
import argparse
import logging
...
def segment_extra(aeeg_output, fs, window_length):
    ...
    for i in range(sec_num - window_length + 1):
        utp.append(np.percentile(aeeg_output[0, i * fs:(window_length + i) * fs], 80))  # 70
        ltp.append(np.percentile(aeeg_output[0, i * fs:(window_length + i) * fs], 50))  # 30
    return utp, ltp
...
def plot_aEEG(utp, ltp, tmp_data_path):
    ...
    plt.savefig(os.path.dirname(tmp_data_path) + '/aEEG0912.png')
...
def filter_multichannel_eeg(eeg_data, fs, numtaps, n_jobs=3):
    filtered = Parallel(n_jobs=n_jobs)(delayed(filter_one_channel)(ch_data, fs, numtaps) for ch_data in eeg_data)
...
def aEEG_compute_h3(sigbufs, channels, sample_frequency, band, window_length):
    ...
    if len(channel_labels) > 0:
        ...
        fs = sample_frequency
        trans_width = 0.4  # Width of transition from pass to stop, Hz
        numtaps = 273  # 2181  # numtaps = filter orders + 1 301
        asym_filter_freq_hz = [0, band[0] - trans_width, band[0], band[1], band[1] + trans_width, 0.5 * fs]
