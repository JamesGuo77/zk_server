...
# python manage.py runserver 0.0.0.0:8040 --noreload
# import sys
import os
import argparse
import logging
...
def asym_filter_envelope1(numtaps, asym_filter_freq_hz, fs, source_eeg):
    ...
    width = 0.4
    nyq = 0.5*fs
    atten = kaiser_atten(numtaps=numtaps,width=width/nyq) #desired width of the transition region between passband and stopband expressed as a fraction of the Nyquist frequency
    beta = kaiser_beta(atten)
    b = firwin(numtaps=numtaps,cutoff=[2, 15],nyq=nyq,pass_zero=False,window=('kaiser', beta), scale=False)
    a = 1

    # filtered signal
    y = lfilter(b, a, source_eeg)  # sigbufs[0, :]
    ...
    aeeg_output = 1.631 * enveloped_eeg + 1  # 2
    return aeeg_output
...
def aEEG_compute(sigbufs, channels, sample_frequency, band, window_length):
    ...
    if len(channel_labels) > 0:
        ...
        # 非对称滤波 整流 振幅放大
        aeeg_output = asym_filter_envelope1(numtaps, asym_filter_freq_hz, fs, source_eeg)
        # 分段提取UTP LTP
        channel_values = segment_extra1(aeeg_output, int(fs), window_length)
...
def aEEG_compute1(input_data_dict, band, window_length):
    ...
    if channles > 0:
        ...
        desired = [1, 3, 18]
        # 非对称滤波 整流 振幅放大
        aeeg_output = asym_filter_envelope2(numtaps, asym_filter_freq_hz, desired, fs, source_eeg)
