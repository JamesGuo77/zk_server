...
trend_name_split = ["ABP", "RBP", "SE", "Envelope", "TP", "ADR", "ABR", "SEF"]
...
def read_bdf1(sigbufs, signal_labels, channels, node_dict):
    ...
    for inx in range(len(channels)):
        ...
        #     sigbufs[inx, 0:col_length] = f.readSignal(chn_inx0, start_inx, stop_inx) - av_dict[chn_split[1]]
            ...
            av_value = np.zeros((sigbufs.shape[0], 1, sigbufs.shape[2]))
...
def abp_rbp_his(train_data, sample_frequency, bp_type):
    band_powers = []

    for segment in train_data:
        # Calculate RBP features for the current epoch
        band_power = multi_relative_band_energy(segment, int(sample_frequency), bp_type)
        band_powers.append(band_power)
...
def abp_rbp_com_t1(train_data, sim_chns, channels, sample_frequency, bp_type, task_id):
    ...
    channel_values = abp_rbp_his(train_data, sample_frequency, bp_type)
...
def envelope_his(sigbufs, special_electrodes, total_chns, channels, fs, task_id):
    # train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
    # start_time1 = time.time()
    # sigbufs_res = sig_uv_read(start_seconds, stop_seconds, special_electrodes, raw, channels, fs, history_monitor)
    # def abp_rbp_com_t1(train_data, sim_chns, channels, sample_frequency, bp_type, task_id):
    # sigbufs = sigbufs[:, [total_chns.index(chn) for chn in sim_chns], :]
    train_data = sig_uv_read2(sigbufs, special_electrodes, total_chns, channels)
...
def multi_chns_tp1(train_data, sim_chns, channels, fs, history_monitor, task_id, band=(0.5, 30), nperseg=None):
    # train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
    train_data = train_data[:, [sim_chns.index(chn) for chn in channels], :]
...
def multi_chns_adr_abr1(train_data, sim_chns, channels, fs, ar_type, task_id, alpha_band=(8, 13), delta_band=(0.5, 4), beta_band=(13,30), nperseg=None):
    # train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
    train_data = train_data[:, [sim_chns.index(chn) for chn in channels], :]
...
def multi_chns_sef1(train_data, sim_chns, channels, fs, task_id, band=(0.5, 30), edge=0.95, nperseg=None):
    # train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
    # start_time1 = time.time()
    train_data = train_data[:, [sim_chns.index(chn) for chn in channels], :]
    # print("读入BDF并处理参考电极幅值完成， %.8s s" % (time.time() - start_time1))
...
def multi_chns_se1(train_data, sim_chns, channels, fs, task_id, band=(0.5, 40), method='welch', nperseg=None, normalize=True):
    # train_data = sig_chns_split(start_seconds, stop_seconds, epoch_length, raw, channels)
    train_data = train_data[:, [sim_chns.index(chn) for chn in channels], :]
    # if history_monitor == 'history':
    #     unit_coef = 1e6
...
# time split
def signal_split(start_seconds, stop_seconds, sigbufs, epoch_length, fs):
    start_time = start_seconds
    # epoch_length = 10
    while start_time <= stop_seconds + 0.01 - epoch_length:  # max(raw.times) = 3600
        temp = sigbufs[:, int(start_time * fs):int((start_time + epoch_length) * fs)]
        temp = temp.reshape((1, temp.shape[0], temp.shape[1]))
...
def signal_read(start_seconds, stop_seconds, raw, history_monitor):
    start, stop = raw.time_as_index([start_seconds, stop_seconds])
    sigbufs = raw[:, start:stop][0]  # [raw.ch_names.index(chn_name) for chn_name in channels]
...
def sig_uv_read2(sigbufs, special_electrodes, total_chns, channels):
    node_dict = special_node(special_electrodes)
    signal_labels = total_chns
    sigbufs_res = read_bdf1(sigbufs, signal_labels, channels, node_dict)  # mne读取信号单位为uV

    return sigbufs_res

def sig_uv_read1(sigbufs, special_electrodes, total_chns, channels):
    node_dict = special_node(special_electrodes)
    signal_labels = total_chns
    sigbufs_res = read_bdf(sigbufs, signal_labels, channels, node_dict)  # mne读取信号单位为uV
...
if __name__ == '__main__':
    ...
    if is_history == 0:
        ...
        ttype = trend_name[6]  # 1 2 3 4 5 6 9 10
        sigbufs = signal_read(start_seconds, stop_seconds, raw, 'history')

        if 1:
            # train_data = sig_chns_split1(start_seconds, stop_seconds, epoch_length, raw, chn_list_sim)
            train_data = signal_split(start_seconds, stop_seconds, sigbufs, epoch_length, int(raw.info['sfreq']))
        ...
        # multi_chns_env(start_seconds, stop_seconds, special_electrodes, raw, ttype, channels, fs, history_monitor,
        #                task_id)
