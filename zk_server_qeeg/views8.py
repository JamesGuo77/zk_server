...
# 2025/10/10 V1.7 aeeg、abp、rbp、se回读多线程测试通过
...
# time split
def sig_chns_split(start_seconds, stop_seconds, raw, channels):
    train_data = []
    start_time = start_seconds
    # epoch_length = 10
    step_size = epoch_length  # 10
...
def abp_rbp_com_t(start_seconds, stop_seconds, raw, channels, sample_frequency, bp_type, task_id):
    ...
    #
    # train_data = np.array(train_data)
    train_data = sig_chns_split(start_seconds, stop_seconds, raw, channels)
    # rbp_powers = []
    # abp_powers = []
    # for segment in train_data:
    #     # Calculate RBP features for the current epoch
    ...
    if request.method == 'POST':
        if request.content_type == 'application/json':
            ...
            for fname in os.listdir(json_data['fileDir']):  # 寻找.bdf文件
                ...
                # abp bp_type: 0-abp and rbp 1-abp 2-rbp
                bp_labels, _, rbp_values = abp_rbp_com(start_seconds, stop_seconds, raw, trend_chns_dict["RBP"], sample_frequency, 2)
    ...
    if request.method == 'POST':
        if request.content_type == 'application/json':
            ...
            for fname in os.listdir(json_data['fileDir']):  # 寻找.rml文件
                ...
                # for ttype in trend_chns_dict.keys():
                #     if ttype == trend_name[0]:  # ["aEEG", "RBP", "ABP", "RAV", "SE", "CSA", "Envelope", "TP", "ADR", "ABR"]
                ...
                aEEG_t = threading.Thread(target=thread_workers, args=(aEEG_com_t, (
                            ...
                            sample_frequency, json_data["taskID"]), result_queue))
                    # # abp bp_type: 0-abp and rbp 1-abp 2-rbp
                abp_t = threading.Thread(target=thread_workers, args=(abp_rbp_com_t, (start_seconds, stop_seconds, raw, trend_chns_sim_dict["ABP"], sample_frequency, 1, json_data["taskID"]), result_queue))
                rbp_t = threading.Thread(target=thread_workers, args=(abp_rbp_com_t, (
                start_seconds, stop_seconds, raw, trend_chns_sim_dict["RBP"], sample_frequency, 2, json_data["taskID"]),
                                                                      result_queue))
                # sef_t = threading.Thread(target=thread_workers, args=(multi_chns_sef, (
                #     start_seconds, stop_seconds, raw, trend_chns_dict["SE"], sample_frequency, json_data["taskID"]),
                #                                                       result_queue))
