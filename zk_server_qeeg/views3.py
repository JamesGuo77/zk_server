...
def segment_extra1(aeeg_output, fs, window_length):  # 80/50, 70/50, 85/55
    # logging.info(aeeg_output.shape)
    sec_num = int(aeeg_output.shape[1] / fs)

    k = 0
    utp, ltp = np.percentile(aeeg_output[:, k * fs:(int(window_length) + k) * fs], 90, axis=1), np.percentile(
        aeeg_output[:, k * fs:(int(window_length) + k) * fs], 55, axis=1)

    if sec_num > 1:
        for i in range(1, int(sec_num) - window_length + 1):
            utp_tmp = np.percentile(aeeg_output[:, i * fs:(int(window_length) + i) * fs], 90, axis=1)
            utp = np.vstack((utp, utp_tmp))
            ltp_tmp = np.percentile(aeeg_output[:, i * fs:(int(window_length) + i) * fs], 55, axis=1)
...
def aEEG_compute_h(sigbufs, channels, sample_frequency, band, window_length, start_time):
    channel_labels = channels
    if len(channel_labels) > 0:
        source_eeg = sigbufs
        fs = sample_frequency
        ...
        source_eeg[np.isnan(source_eeg)] = 0
        # bp_fir = firwin(2181, cutoff=[2, 70], fs=fs, pass_zero=False)
        # eeg_filtered = filtfilt(bp_fir, [1.0], source_eeg)
        taps = remez(numtaps, asym_filter_freq_hz, [0, 1, 0], type='bandpass', fs=fs)
        y = lfilter(taps, 1, np.apply_along_axis(remove_spikes, axis=1, arr=source_eeg))
        aeeg_output = 1.231 * np.abs(y) + 4

        fs_new = 250  # 新的采样率
        decimation_factor = int(fs / fs_new)
        # 降采样到250Hz
        aeeg_output = aeeg_output[:, ::decimation_factor]
        print("6th: ", time.time() - start_time)
        # 分段提取UTP LTP
        channel_values = segment_extra1(aeeg_output, fs_new, window_length)
        print("7th: ", time.time() - start_time)
        # aEEG波形
        # plot_aEEG(utp, ltp, tmp_data_path)
    ...
    if request.method == 'POST':
        if request.content_type == 'application/json':
            ...
            for fname in os.listdir(json_data['fileDir']):  # 寻找.rml文件
                ...
                channel_labels, channel_values = aEEG_compute_h(sigbufs_res, channels, sample_frequency, band, window_length, start_time)  # utp, ltp
